...
def main() -> int:
    ...
    runtime_sub = runtime_cmd.add_subparsers(dest="runtime_cmd", required=True); runtime_sub.add_parser("list")
    seed = runtime_sub.add_parser("seed", help="merge missing starter templates without overwriting local entries"); seed.add_argument("--refresh-templates", action="store_true")
    add = runtime_sub.add_parser("add", help="add or patch any local AI runtime")
    add.add_argument("name"); add.add_argument("--display-name"); add.add_argument("--commands-json"); add.add_argument("--entries-json"); add.add_argument("--skills-json"); add.add_argument("--capabilities-json"); add.add_argument("--skills-sync-mode", choices=["per-skill-link", "central-dir-link", "none"]); add.add_argument("--auto-sync-skills", action=argparse.BooleanOptionalAction, default=None); add.add_argument("--review-args-json"); add.add_argument("--review-priority", type=int); add.add_argument("--no-detect", action="store_true")
    remove = runtime_sub.add_parser("remove"); remove.add_argument("name")
    for action in ("enable", "disable"):
        p = runtime_sub.add_parser(action); p.add_argument("name")
    ...
    if args.cmd == "runtime":
        ...
        if args.runtime_cmd == "list":
            print(json.dumps(runtime_summary(read_kind("runtime")), ensure_ascii=False, indent=2)); return 0
        if args.runtime_cmd == "seed":
            holder = {"changed": False}
            def seed_mut(runtime: dict) -> None: holder["changed"] = seed_templates(runtime, force=args.refresh_templates)
            update_kind("runtime", seed_mut); print("runtime templates merged" if holder["changed"] else "runtime templates already seeded"); return 0
        if args.runtime_cmd == "add":
            def add_mut(runtime: dict) -> None:
                register_runtime(runtime, args.name, display_name=args.display_name, command_candidates=_optional_json_list(args.commands_json, parser, "--commands-json"), entry_candidates=_optional_json_list(args.entries_json, parser, "--entries-json"), skills_candidates=_optional_json_list(args.skills_json, parser, "--skills-json"), capabilities=_optional_json_list(args.capabilities_json, parser, "--capabilities-json"), skills_sync_mode=args.skills_sync_mode, auto_sync_skills=args.auto_sync_skills, review_args=_optional_json_list(args.review_args_json, parser, "--review-args-json"), review_priority=args.review_priority)
        ...
        if args.runtime_cmd == "remove":
            ...
            def remove_mut(runtime: dict) -> None:
                ...
                if entries[args.name].get("source") == "template":
                    holder["template"] = True; entries[args.name]["enabled"] = False; entries[args.name]["template_suppressed"] = True
    ...
    if args.cmd == "show":
        print(json.dumps(read_kind(args.kind), ensure_ascii=False, indent=2)); return 0
    if args.cmd == "get":
    ...
    if args.cmd == "unset":
        holder = {"changed": False}
        def unset_mut(data: dict) -> None: holder["changed"] = unset_value(data, args.key)
        update_kind(args.kind, unset_mut); return 0 if holder["changed"] else 2
    return 1
