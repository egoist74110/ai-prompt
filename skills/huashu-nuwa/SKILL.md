---
name: huashu-nuwa
description: Distill any person or theme into a runnable perspective Skill: deep multi-agent research -> mental-model extraction -> SKILL.md generation. Two entries: (1) explicit name -> direct distillation; (2) fuzzy need -> diagnose, recommend candidates, then distill. Triggers: "distill [person]", "nuwa", "create a [person] perspective skill", "how does [person] think", "make a [person] perspective", "update [person]'s skill", or fuzzy needs like "I want to improve decision quality" / "I need a thinking advisor".
---
...
**触发词 / Triggers**：「造skill」「蒸馏XX」「女娲」「造人」「XX的思维方式」「做个XX视角」「更新XX的skill」；模糊需求同样触发：「我想提升决策质量」「有没有一种思维方式能帮我...」「我需要一个思维顾问」；English: "distill [person]", "nuwa", "create a [person] perspective skill", "how does [person] think", "I need a thinking advisor".
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查技能目录：本能力库根目录下的 `skills/`，或运行时的技能目录如 `.claude/skills/`）
...
**来源A：本地已有Skill**
扫描技能目录下 `*-perspective/` 目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
Phase 1启动前，**主动扫描技能目录**（本能力库为根目录下的 `skills/`），检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `skills/[person-name]-perspective/SKILL.md`（本能力库）或对应运行时的技能目录。
